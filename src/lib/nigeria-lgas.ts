// Complete list of Nigeria's 36 States + FCT with all Local Government Areas
export interface NigeriaState {
  state: string
  lgas: string[]
}

export const NIGERIA_STATES_LGAS: NigeriaState[] = [
  {
    state: "Abia",
    lgas: ["Aba North","Aba South","Arochukwu","Bende","Ikwuano","Isiala Ngwa North","Isiala Ngwa South","Isuikwuato","Obi Nwa","Ohafia","Osisioma","Ugwunagbo","Ukwa East","Ukwa West","Umuahia North","Umuahia South","Umu Nneochi"],
  },
  {
    state: "Adamawa",
...
export const STATE_NAMES = NIGERIA_STATES_LGAS.map((s) => s.state)

export function getLGAsForState(stateName: string): string[] {
  return NIGERIA_STATES_LGAS.find((s) => s.state === stateName)?.lgas ?? []
}
